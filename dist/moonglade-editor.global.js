...
var MoongladeEditor = (() => {
  ...
  __export(index_exports, {
    MoongladeEditor: () => MoongladeEditor,
    createCommands: () => createCommands,
    createMoongladeEditor: () => createMoongladeEditor,
    moongladeSchema: () => moongladeSchema,
    parseHtml: () => parseHtml,
    roundTripHtml: () => roundTripHtml,
  ...
  function setLink(markType, href, title) {
    ...
    return (state, dispatch) => {
      ...
      if (dispatch) {
        ...
        transaction.removeMark(markFrom, markTo, markType).addMark(markFrom, markTo, markType.create({ href: safeHref, title: title?.trim() || null }));
      ...
      if (dispatch) {
        ...
        if (typeof tablePos !== "number") {
          transaction.doc.descendants((node, pos) => {
            if (node.type === schema2.nodes.table) {
              tablePos = pos;
              return false;
            }
            return true;
...
${INDENT.repeat(depth)}</${node.tagName.toLowerCase()}>`;
  ...
  // src/schema.ts
  var nodes2 = addListNodes(
    schema.spec.nodes.update("paragraph", withAlignment(schema.spec.nodes.get("paragraph"))).update("heading", withAlignment(schema.spec.nodes.get("heading"))).update("code_block", withCodeLanguage()),
    "paragraph block*",
    "block"
  ).append(
    tableNodes({
  ...
  var marks2 = schema.spec.marks.append({
    underline: {
      parseDOM: [
        { tag: "u" },
        {
  ...
  function withAlignment(spec) {
    return {
      ...
      toDOM(node) {
        ...
        const attrs = typeof dom[1] === "object" && !Array.isArray(dom[1]) ? { ...dom[1] } : {};
        attrs.style = `${attrs.style || ""}text-align: ${align};`;
        return typeof dom[1] === "object" && !Array.isArray(dom[1]) ? [dom[0], attrs, ...dom.slice(2)] : [dom[0], attrs, ...dom.slice(1)];
      }
    };
  }
  ...
  function createCodeDialog(commands, actions) {
    const root = document.createElement("div");
    root.className = "mg-editor-dialog dropdown-menu show p-3 shadow";
    root.hidden = true;
    root.setAttribute("role", "dialog");
  ...
  // src/toolbar.ts
  function createToolbar({ schema: schema2, commands, uploadConfigured, actions }) {
    const root = document.createElement("div");
    ...
    formatSelect.addEventListener("change", () => {
      ...
      actions.execute(type === "heading" ? commands.heading(Number(level)) : commands.paragraph);
    });
    const buttons = {};
    const colorDropdowns = [];
    const localCloseColorDropdowns = (except) => {
      closeColorDropdowns({ colorDropdowns }, except);
    };
    const localToggleColorDropdown = (dropdown) => {
    ...
    colorGroup.append(
      createColorDropdown({
        label: "Text color",
        symbol: "A",
        markType: schema2.marks.text_color,
      ...
      createColorDropdown({
        ...
        symbol: "ab",
        markType: schema2.marks.background_color,
  ...
  function createColorDropdown({
    ...
    colorDropdowns,
    actions,
    closeColorDropdowns: closeColorDropdowns2,
    toggleColorDropdown
  }) {
    const group = document.createElement("div");
...
//# sourceMappingURL=moonglade-editor.global.js.map
